...
app.use(
  cors({
    origin: [
      "http://localhost:5173",
      "https://screenmates-r78w.vercel.app",
      "http://localhost:8000",
      "http://localhost:4173",
    ],
    credentials: true,
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    ...
    if (userRoom && !userRoom.participants.length) {
      GroupRooms[socket.id].participants = userRoom.participant.splice(
        userRoom.participants.findIndex(connectedUsers_IDtoUserName[socket.id]),
        1
      );
